		...
		public void paintComponent(Graphics g) {
			...
			for (int i = 0; i < money.length; i++) {
				if (money[i] > 0) {
					...
					AlgorithmVisHelper.fillRectangle(g2d, i * w + 1, canvasHeight/2 - money[i], w - 1, money[i]);
				} else {
					AlgorithmVisHelper.setColor(g2d, AlgorithmVisHelper.Pink);
					AlgorithmVisHelper.fillRectangle(g2d, i * w +  1, canvasHeight/2, w - 1, -money[i]);
				}
			}
		}
